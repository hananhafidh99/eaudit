...
<body onload="window.print()">
     ...
     <div id="colres" >
        ...
        TAHUN ANGGARAN 2023<br>Kode Rekening {{ $penugasan['norek'] }}</h2></div><br><div><table border='0' >
        ...
        <tr><td >Untuk &emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;</td><td> : </td><td> {{ $penugasan['nama_jenispengawasan'] }} ke {{ $penugasan['nama_obrik'] }}</td></tr>
        <tr><td >Tanggal Mulai </td><td>:</td><td>{{ Carbon\Carbon::parse($penugasan['tanggalAwalPenugasan'])->translatedFormat('d F Y') }}</td></tr>
        <tr ><td >Tanggal Akhir</td><td>:</td><td>{{ Carbon\Carbon::parse($penugasan['tanggalAkhirPenugasan'])->translatedFormat('d F Y') }}</td></tr>
        ...
        <div id="lead" class="e">
        <p class="tgh">Sragen, {{ Carbon\Carbon::parse($penugasan['tanggalAkhirPenugasan'])->translatedFormat('d F Y') }}</p>
        </div>
        <table>
         <tr>
          <td class="tgh" colspan="4" width="350"><p style="margin-left: -220px">Pengguna Anggaran</p>
            <br><br><br><p style="margin-left: -130px">{{session('nama_pemimpin')}}</p>  <p style="margin-left: -155px; margin-top:-20px">NIP. {{ session('nip_pemimpin') }}</p> </td>
           ...
           <p style="margin-left: -10px">{{ session('nama_bendahara') }}</p> <p style="margin-left: -5px; margin-top:-15px">NIP. {{ session('nip_bendahara') }}</p> </td>
